...
export default function Explore() {
  ...
  useEffect(() => {
    fetchStats().then((s) => {
      const stats = s.stats
      if (chainFilter === 'all') setTotalCount(Object.values(stats).reduce((sum, s) => sum + s.collections, 0))
      else if (stats[chainFilter]) setTotalCount(stats[chainFilter].collections)
    }).catch(() => {})
  }, [chainFilter])
  ...
  useEffect(() => {
    ...
    if (chainFilter === 'all') {
      ...
      }).catch(() => { setCollections([]); setAllCollections([]) })
        .finally(() => setLoading(false))
    } else {
      fetchRecentlyActive(chainFilter, BATCH_SIZE)
        .then((cols) => { setCollections(cols); setAllCollections(cols) })
        .catch(() => { setCollections([]); setAllCollections([]) })
        .finally(() => setLoading(false))
    }
  }, [chainFilter])
  ...
  const loadMore = useCallback(async () => {
    if (query.length >= SEARCH_MIN_CHARS || loadingMore) return
    setLoadingMore(true)
  ...
  }, [chainFilter, query, allCollections, visibleCount, loadingMore])
    ...
    <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-10">
      ...
      <div className="flex items-center justify-between mb-5">
        <span className="text-xs text-muted-foreground font-mono">
          {loading || searching ? '...' : `${visible.length} of ${totalCount ?? collections.length} collections`}
        </span>
      </div>
        ...
        <>
          ...
          {hasMore && (
            <div className="flex items-center justify-center gap-3 py-8">
              ...
              {loadingMore && <CircleNotch size={20} className="animate-spin text-primary" />}
            </div>
          )}
        </>
      ) : (
